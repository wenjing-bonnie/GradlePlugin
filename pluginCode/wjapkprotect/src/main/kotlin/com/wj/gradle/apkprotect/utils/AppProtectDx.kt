...
object AppProtectDx {
    ...
    fun jar2Dex(aarFile: File, project: Project): File {
        ...
        return jar2Dex(aarFile, File(unzipDirectory))
    }
     ...
     * 执行dx命令
     */
    private fun dxCommand(aarDex: File, classJar: File) {
        val runtime = Runtime.getRuntime()
        val process =
            runtime.exec("dx --dex --output=${aarDex.absolutePath} ${classJar.absolutePath}")
        try {
            process.waitFor()
